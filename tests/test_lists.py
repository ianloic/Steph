...
class ListTests(StephTest):
    ...
    def test_three_numbers_list(self):
        p = parse('[1, 2, 3]')
        self.assertIsInstance(p, ast.List)
        self.assertEqual(p.type, typesystem.List(ast.number.Number()))
    ...
    def test_identity_function(self):
        ...
        (l:List(Number)) => l
        ''')
        number_list = typesystem.List(ast.number.Number())
    ...
    def test_wrap_function(self):
        ...
        (l:List(Number)) => [l]
        ''')
        self.assertEqual(p.type, typesystem.Function([typesystem.List(ast.number.Number())],
                                                     typesystem.List(typesystem.List(ast.number.Number()))))
